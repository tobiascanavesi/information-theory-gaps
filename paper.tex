...
We also verify that the empirical single-gap entropy satisfies $H_1 \approx \log_2(\log x) + C$ with $C \approx 0.44$,
...
\begin{proof}
In the Cram\'er model with constant density $1/\log x$, the gap $g_n = p_{n+1} - p_n$ is determined by the primality of integers $p_n + 1, p_n + 2, \ldots$, which are independent. Given $p_{n+1}$, the gap $g_{n+1}$ depends only on the primality of integers $p_{n+1} + 1, p_{n+1} + 2, \ldots$, a disjoint set of integers. Since primality assignments are independent in the Cram\'er model, $g_{n+1}$ is conditionally independent of $g_n$ given $p_{n+1}$, and since the density is approximately constant near $x$, this extends to marginal independence. More precisely, gaps follow a geometric distribution $\Pr(g = k) \approx (1 - 1/\log x)^{k-1} / \log x$ for $k \ge 1$, and by the memoryless property of the geometric distribution, the waiting time beyond $p_{n+1}$ is independent of the waiting time to reach $p_{n+1}$.
\end{proof}
...
The sieve creates correlations through the Hardy--Littlewood singular series: the probability of a gap of size $h$ depends on the prime factorization of $h$ via $\fS(\{0, h\}) = 2C_2 \prod_{p \mid h,\, p > 2} (p-1)/(p-2)$. Since consecutive gaps $g_n = h_1$ and $g_{n+1} = h_2$ involve three primes $(p_n, p_n + h_1, p_n + h_1 + h_2)$, their joint probability depends on the \emph{triple} singular series $\fS(\{0, h_1, h_1 + h_2\})$, which generally does not factor as $\fS(\{0, h_1\}) \cdot \fS(\{0, h_2\})$.
This non-factorization is the source of the mutual information.
\end{observation}
...
\item The residual $C$ in $H_1 \approx \log_2(\log x) + C$ decreases from $0.44$ to $0.28$ (Table~\ref{tab:h1-scaling}).
\end{itemize}
...
\begin{enumerate}
\item \textbf{Compute $\MI$ under the Hardy--Littlewood model}: derive an asymptotic expression for $\MI(g_n; g_{n+1})$ using the triple singular series $\fS(\{0, h_1, h_1 + h_2\})$ and compare to the empirical $0.33$ bits. The discrepancy would quantify higher-order sieve effects.
