...
We can compute $\MI_{\text{H-L}}$ numerically by summing~\eqref{eq:hl-marginal} and~\eqref{eq:hl-joint} over all gap pairs $(h_1, h_2)$ up to $\sim 8 \log x$, computing the marginal and joint entropies, and taking their difference.
For the triple singular series, we use the product formula
\[
\fS(\{0, h_1, h_2\}) = \prod_{p} \frac{(1 - \nu_p / p)}{(1 - 1/p)^3},
\]
where $\nu_p = |\{0, h_1, h_2\} \bmod p|$ counts distinct residues, with the product over all primes~$p$.
